...
object Interp {
  ...
  def interp(g: Node, verbose: Boolean = false): Value = {
    try {
      interp(Graph.emptyIdentityMap, g, verbose)
  ...
  def interp(env: Env, n0: Node, verbose: Boolean): Unit = {
    var indent = 0
